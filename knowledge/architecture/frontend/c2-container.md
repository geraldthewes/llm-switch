...
```mermaid
C4Container
    title llm-switch Frontend Container Architecture
    Container_Boundary(api_zone, "API Zone") {
        Container(frontend, "llm-switch API<br/>Backend", "Golang, bifrost, Docker", "Intelligent model routing proxy\nOpenAI/Anthropic compatible APIs")
        Container(code_review, "Code Review Tool", "TypeScript, React", "AI-powered code analysis application")
        Container(chat_app, "Chat Application", "Python, Streamlit", "Conversational AI interface")
        Container(data_tool, "Data Analysis Tool", "Jupyter, Pandas", "Interactive data exploration notebook")
        Container(local_model, "Local Model Server", "vLLM, Docker, GPU", "Serves Qwen/Nemotron models\nFramework: vLLM or llama.cpp")
        Container(frontier_model, "Frontier Model API", "HTTPS, REST", "External API (OpenAI/Anthropic)\nAccessed via managed credentials")
    }
    Container_Ext(consul, "Consul", "Service Discovery, DNS", "Service registration and health checks")
    Container_Ext(vault, "Vault", "Secret Management", "Secure storage for API keys\nand configuration secrets")
    Container_Ext(nomad, "Nomad", "Orchestration, Scheduler", "Job scheduling and container management\n3-node cluster deployment")
    
    Rel(code_review, frontend, "POST /v1/chat/completions<br>POST /v1/messages", "HTTP/1.1, JSON")
    Rel(chat_app, frontend, "POST /v1/chat/completions<br>POST /v1/messages", "HTTP/1.1, JSON")
    Rel(data_tool, frontend, "POST /v1/completions<br>POST /v1/embeddings", "HTTP/1.1, JSON")
    Rel(frontend, local_model, "Route based on complexity<br>NormStat/VecStat classification", "gRPC, TCP")
    Rel(frontend, frontier_model, "Fallback for complex tasks<br>Cost-optimized routing", "HTTPS, TLS 1.3")
    Rel(frontend, consul, "Service registration<br>Health check propagation", "Consul API, DNS")
    Rel(frontend, vault, "Retrieve API keys<br>Fetch model credentials", "Vault API, TLS")
    Rel(frontend, nomad, "Job deployment<br>Resource allocation", "Nomad SDK, HTTP/1.1")
    Rel(consul, vault, "Secrets sync<br>Configuration updates", "Consul Template, HTTPS")
    Rel_Back(nomad, frontend, "Health status<br>Resource metrics", "Nomad SDK, HTTP/1.1")
