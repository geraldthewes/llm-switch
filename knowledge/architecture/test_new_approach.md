...
C4Context
    ...
    System_Boundary(llmSwitch, "llm-switch", "Intelligent LLM proxy\nGolang 1.21+, Docker 24.0+\nProvides Prometheus metrics at /metrics, Grafana dashboard llm-switch-overview, hardware-aware routing (memory-priority), respects Kubernetes Node Affinity for GPU scheduling") {
    ...
    Rel_Back(llmSwitch, frontierAPI, "HTTPS: llm-switch fails over to frontier API (<200ms>)", "HTTPS")
    Rel_Back(llmSwitch, vault, "HTTPS: llm-switch refreshes token with Vault (<100ms>)", "HTTPS")
